...
def max_lyapunov_exponent(data, emb_dim: int = 6, lag: int = 1, min_tsep: int | None = None) -> dict:
    ...
    if nolds is None:
        return {"available": False, "value": float("nan"), "reason": "lyapunov_backend_unavailable"}
    try:
        ...
        return {"available": False, "value": float("nan"), "reason": "lyapunov_estimation_failed", "error": str(exc)}
